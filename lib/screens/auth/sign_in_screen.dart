...
import 'account_profile_loading_screen.dart';
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  /// Authenticates, clears the prior account, then starts loading the new one.
  Future<void> _signIn() async {
    ...
    late final String signedInUserId;
    try {
      signedInUserId = await auth.signInWithEmailPassword(
    ...
    // Firebase now belongs to a different user. Clear every in-memory value
    // owned by the prior account before awaiting anything else or checking
    // whether this widget is still mounted.
    appState.clearForAccountChange();

    if (!mounted) return;
    // Profile loading happens only after the old navigation stack is gone.
    // Its retry screen is tied to the user id established above.
    navigator.pushAndRemoveUntil(
      MaterialPageRoute<void>(
        builder: (BuildContext context) =>
            AccountProfileLoadingScreen(expectedUserId: signedInUserId),
      ),
